...
class ConformalModel(nn.Module):
    def __init__(self, model, calib_loader, args, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, pct_paramtune = 0.3, batch_size=32, lamda_criterion='size'):
        super(ConformalModel, self).__init__()
        self.model = model 
        self.alpha = alpha
        self.T = torch.Tensor([1.3]) #initialize (1.3 is usually a good value)
        self.T, calib_logits = platt(self, calib_loader, args)
        ...
        if kreg == None or lamda == None:
            print('RAPS')
            kreg, lamda, calib_logits = pick_parameters(model, args, calib_logits, alpha, kreg, lamda, randomized, allow_zero_sets, pct_paramtune, batch_size, lamda_criterion)
...
# Temperature scaling
def platt(cmodel, calib_loader, args, max_iters=10, lr=0.01, epsilon=0.01):
    print("Begin Platt scaling.")
    # Save logits so don't need to double compute them
    if args.use_adv_calib == True:
        logits_dataset = get_logits_targets_adv(cmodel.model, calib_loader, args)
    else:
        logits_dataset = get_logits_targets(cmodel.model, calib_loader, args)
...
class ConformalModelLogits(nn.Module):
    def __init__(self, model, calib_loader, args, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, naive=False, LAC=False, pct_paramtune = 0.3, batch_size=32, lamda_criterion='size'):
        ...
        if (kreg == None or lamda == None) and not naive and not LAC:
            kreg, lamda, calib_logits = pick_parameters(model, args, calib_loader.dataset, alpha, kreg, lamda, randomized, allow_zero_sets, pct_paramtune, batch_size, lamda_criterion)
            calib_loader = tdata.DataLoader(calib_logits, batch_size=batch_size, shuffle=False, pin_memory=True)
...
def pick_lamda_size(model, args, paramtune_loader, alpha, kreg, randomized, allow_zero_sets):
    ...
    for temp_lam in [0.001, 0.01, 0.1, 0.2, 0.5]: # predefined grid, change if more precision desired.
        conformal_model = ConformalModelLogits(model, paramtune_loader, args, alpha=alpha, kreg=kreg, lamda=temp_lam, randomized=randomized, allow_zero_sets=allow_zero_sets, naive=False)
        if args.use_adv_calib == True:
...
def pick_parameters(model, args, calib_logits, alpha, kreg, lamda, randomized, allow_zero_sets, pct_paramtune, batch_size, lamda_criterion):
    ...
    if lamda == None:
        if lamda_criterion == "size":
            lamda = pick_lamda_size(model, args, paramtune_loader, alpha, kreg, randomized, allow_zero_sets)
